...
<script>
	export default {
		...
		methods: {
			...
			onPointerUp(e) {
				if (this.activeEl) {
					try { this.activeEl.releasePointerCapture(e.pointerId); } catch {}
					this.activeEl = null;
				}
		...
		computed: {
			progress() {
				return (this.isVideoDragging ? this.draggingTime : this.currentTime) / (this.duration || 1);
			}
		}
	}
</script>
...
<style>
	...
	#biliVideoControls:hover { opacity: 1; text-shadow: 0 0 10px rgba(0, 0, 0, 0.5); }
	.videoName-container { overflow: hidden; white-space: nowrap; position: relative; }
	#videoName { display: inline-block; padding-left: 100%; animation: scroll-left 15s linear infinite; }
	.videoName-container:hover #videoName { animation-play-state: paused; }
	@keyframes scroll-left { 0% { transform: translateX(0); } 100% { transform: translateX(-100%); } }
	#controls { display: flex; align-items: center; justify-content: space-evenly; }
	#controls i { font-size: 10vh; height: 10vh; line-height: 10vh; }
	.audio-control { position: relative; display: inline-block; cursor: pointer; }
	.audio-control .volume-slider {
		position: absolute; bottom: 100%; left: 50%; transform: translateX(-50%);
		width: 30px; height: 120px; background: rgba(255,255,255,0.8); border-radius: 4px;
		padding: 8px 5px; opacity: 0; visibility: hidden; transition: opacity 0.2s, visibility 0.2s;
		display: flex; flex-direction: column; align-items: center; margin-bottom: 8px;
	}
	.audio-control:hover .volume-slider { opacity: 1; visibility: visible; }
	.show { opacity: 1 !important; visibility: visible !important; }
	.volume-track { width: 4px; height: 100%; background: rgba(255,255,255,0.3); border-radius: 2px; position: relative; touch-action: none; }
	.volume-progress { position: absolute; bottom: 0; left: 0; width: 100%; height: 70%; background: white; border-radius: 2px; transition: height 0.1s ease; }
	.volume-thumb { position: absolute; left: 50%; bottom: 70%; transform: translate(-50%, 50%); width: 12px; height: 12px; background: white; border-radius: 50%; cursor: pointer; box-shadow: 0 0 2px rgba(255,255,255,0.3); transition: bottom 0.1s ease; }
	.volume-number { font-size: 2vh; }
	.time { font-size: 14px; min-width: 60px; text-align: center; }
	.progressContainer { display: flex; align-items: center; gap: 5px; height: 5px; margin-top: 15px; position: relative; }
	.progressWrapper { flex: 1; height: 5px; background-color: rgba(255,255,255,0.3); position: relative; touch-action: none; cursor: pointer; }
	.progressBar { height: 100%; background-color: rgba(255,255,255,0.8); position: relative; }
	.progressHandle { position: absolute; right: -5px; top: 50%; transform: translateY(-50%); width: 15px; height: 15px; background-color: #fff; border-radius: 50%; box-shadow: 0 0 5px rgba(255,255,255,0.5); display: none; }
	#biliVideoControls:hover .progressHandle { display: block; }
</style>
