...
async function searchSong(keyword, retryCount = 0) {
...
async function resolveVideoUrl(bvid) {
  ...
  if (mixinKey) {
    try {
      ...
      if (playData.code === 0 && playData.data?.durl?.length > 0) {
        return { videoUrl: playData.data.durl[0].url, viewData, playData };
      }
    } catch {
      clearMixinKey();
    }
  ...
  if (playData2.data?.durl?.length > 0) {
    return {
      videoUrl: playData2.data.durl[0].url,
      viewData,
      playData: playData2,
    };
  }
  ...
  if (dashData.code === 0 && dashData.data?.dash?.video?.length > 0) {
    return {
      videoUrl: dashData.data.dash.video[0].baseUrl,
      viewData,
      playData: dashData,
    };
  }
